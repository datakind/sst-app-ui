...
export default function ResponsiveNavLink({ active, href, children, ...props }) {
    ...
    return (<div>
      {'as' in props && props.as === 'button' ? (<button className={classNames('w-full text-left', classes)}>
          {children}
        </button>) : (<Link href={href || ''} className={classes}>
          {children}
        </Link>)}
    </div>);
}
